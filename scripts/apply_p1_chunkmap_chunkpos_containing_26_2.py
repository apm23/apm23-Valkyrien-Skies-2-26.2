...
old_a = "new ChunkPos(BlockPos.containing(VSGameUtilsKt.toWorldCoordinates(level, arg.getMiddleBlockPosition(63)))).toLong()"
new_a = "ChunkPos.containing(BlockPos.containing(VSGameUtilsKt.toWorldCoordinates(level, arg.getMiddleBlockPosition(63)))).pack()"
...
for old, new, label in [
    (old_a, new_a, "nearby packed-key conversion"),
    (old_b, new_b, "distance ChunkPos conversion"),
]:
    if text.count(old) != 1:
...
# Preserve upstream VS2 world-space conversion and wrapped vanilla call semantics exactly.
anchors = [
