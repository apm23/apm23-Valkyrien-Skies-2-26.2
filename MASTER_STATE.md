...
## Current reconciliation — AssemblyUtil blockUpdated neighbor dispatch boundary proven and frozen

- Current proven implementation HEAD: `12f182325f9f3e10c502afae7da567e4b1b95c28` (`P1: wire AssemblyUtil blockUpdated neighbor proof`).
- Exact-head P0 provenance run `35438749400`, job `105885804198`, completed `success`; the exact upstream gitlink remained `f39132148e717d325933b4ce6e9e9fb13d929390`.
- Exact-head P1 standalone compile run `35438749392`, job `105885804160`, completed `failure` only because independent Minecraft 26.2 source/API clusters remain.
- Every overlay step through this proof, including `Apply traceable P1 AssemblyUtil blockUpdated neighbor overlay`, `Show and validate port delta`, Gradle runtime setup, and diagnostic artifact upload completed successfully; compilation reached the real `:common:compileKotlin` boundary.
- Root cause proven for this isolated cluster: pinned upstream 1.21.1 `AssemblyUtil.updateBlock()` calls `Level.blockUpdated(BlockPos, Block)` twice as part of its explicit update sequence. The legacy runtime dispatch is not an unconditional neighbor update: client `Level` does not own the server neighbor-fanout behavior, while `ServerLevel.blockUpdated` suppresses it in debug worlds and otherwise forwards to `updateNeighborsAt`. Minecraft 26.2 no longer exposes the old `blockUpdated` source boundary used here, but retains `Level.isClientSide`, `Level.isDebug()`, and `updateNeighborsAt(BlockPos, Block)`. The behavior-preserving adaptation therefore makes the prior dispatch explicit at only the two pinned sites: `if (!level.isClientSide && !level.isDebug()) { level.updateNeighborsAt(...) }`.
- `scripts/apply_p1_assemblyutil_blockupdated_26_2.py` is fail-closed and targets exactly the two pinned-upstream neighbor-update contexts in:
  - `common/src/main/kotlin/org/valkyrienskies/mod/common/assembly/AssemblyUtil.kt`
- The overlay converts only `level.blockUpdated(fromPos, AIR.block)` and `level.blockUpdated(toPos, toState.block)` to the explicit non-client/non-debug `updateNeighborsAt(...)` dispatch. It requires exactly one pinned context for each site, exactly two server/non-debug guards, exactly one resulting current API call per site, rejects any remaining `level.blockUpdated(` in `AssemblyUtil`, and verifies adjacent update phases remain present exactly once.
- This proof preserves the existing `setBlocksDirty`, `sendBlockUpdated`, `AIR.updateIndirectNeighbourShapes`, `AIR.updateNeighbourShapes`, light-engine `checkBlock`, analog-output `updateNeighbourForOutputSignal`, flags, recursion depth, source/destination blocks and positions, and update ordering. It does **not** change block placement/removal, block-entity/component persistence, scheduled ticks, relocation, assembly order, ship allocation, transforms, physics, collision, networking, gameplay authority, or camera behavior.
- Exact run `35438749392` contains **no compiler diagnostic for `AssemblyUtil.kt` at all**. The former line-68/78 `blockUpdated` diagnostics are cleared and no new `isDebug()` / `updateNeighborsAt(...)` diagnostic appeared. This proof clears only the `AssemblyUtil` neighbor-dispatch boundary and does not make overall P1 green.
- Diagnostic artifact: `p1-compile-log-12f182325f9f3e10c502afae7da567e4b1b95c28`, artifact ID `10583103574`, size `5416` bytes, ZIP SHA-256 `5d61e12c76f80d30260873150485a5dedf10375ee5bcafa8fcd0580d7fc05ffd`.
- Scope reconciliation from prior ledger checkpoint `10a8f4a59cc87c66dfb8cd854d15ec7ffa3b75ab` to the proven implementation HEAD changed only the new fail-closed AssemblyUtil blockUpdated-neighbor overlay and its canonical P1 workflow wiring; the pinned `upstream-vs2` gitlink was untouched.
- The similar `blockUpdated` diagnostic still present in `RelocationUtil` is **not** covered by this proof. That file combines relocation persistence/component loading, loot-key handling, update flags, and relocation semantics and must be inspected as a separate cluster rather than mechanically copying this bridge.
...
The immediately prior implementation proof is `644d108b111bed17d0f3259dc840803f7c9a3192` (AssemblyUtil block-entity Value I/O boundary). Its proof and all earlier historical proof records/failed probes remain frozen evidence and must not be replayed merely because a later compiler error resembles them.
...
- active_proof_head: `12f182325f9f3e10c502afae7da567e4b1b95c28; AssemblyUtil blockUpdated neighbor dispatch boundary complete and frozen`
- active_proof_run: `P0 35438749400 / job 105885804198 success; P1 35438749392 / job 105885804160 failure with all AssemblyUtil diagnostics cleared; independent compiler clusters remain`
...
## Remaining compiler areas from exact run `35438749392`

These remain unresolved and independent from the frozen AssemblyUtil neighbor-dispatch proof:
...
- `RelocationUtil`: ValueInput/ValueOutput/component loading, loot-key nullability, integer update-flag migrations, its own removed `blockUpdated` sites, and related relocation semantics. The frozen AssemblyUtil neighbor bridge does not authorize copying a patch here without inspecting the full relocation boundary.

`AssemblyUtil.kt` has **no remaining compiler diagnostic in exact run `35438749392`**. `VSGameUtils.kt`, `VSGameEvents.kt`, the `AbstractArrow` / `AbstractHurtingProjectile` package sites, all three proven `ShipMountingEntity` adaptation boundaries, the `ShipAssembler.getDistinctChunksFromBlockPosSet()` containing-chunk conversion, the two `AssemblyUtil` direct chunk-write flag arguments, the `AssemblyUtil` ScheduledTick explicit generic bound, the `AssemblyUtil` block-entity Value I/O bridge, and the `AssemblyUtil` blockUpdated neighbor-dispatch bridge are **not** remaining compiler areas after their frozen proofs.
