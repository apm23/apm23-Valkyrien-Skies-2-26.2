...
## Current reconciliation — RelocationUtil pending loot-table clear proven and frozen

- Current proven implementation HEAD: `52a05bdae75fd2a469287234c680bf191fd5b105` (`P1: wire RelocationUtil loot clear proof`).
- The implementation consists of fail-closed overlay commit `4cd619fa2d01a6c71023023c21b22547f77c48c6` plus canonical P1 workflow wiring at `52a05bdae75fd2a469287234c680bf191fd5b105`; no upstream submodule commit or gameplay implementation was replaced.
- Exact-head P0 provenance run `35444914843`, job `105902069944`, completed `success`; the exact upstream gitlink remained `f39132148e717d325933b4ce6e9e9fb13d929390`.
- Exact-head P1 standalone compile run `35444914831`, job `105902069897`, completed `failure` only because independent Minecraft 26.2 source/API clusters remain. The RelocationUtil loot-table-clear overlay step, port-delta validation, Gradle runtime setup, and compile-log artifact upload completed successfully.
- Diagnostic artifact: `p1-compile-log-52a05bdae75fd2a469287234c680bf191fd5b105`, artifact ID `10584832788`, size `5060` bytes, ZIP SHA-256 `51c13dabc67ec11ff92c55e3c767b5d5aa3df9f52b81b8c12b9585caf916de34`.
- Root cause proven for this isolated cluster: pinned upstream `RelocationUtil.relocateBlock()` clears a `RandomizableContainerBlockEntity` pending loot table with legacy `setLootTable(null, 0)` before removing the source block entity, preserving the upstream intent that a deferred loot container being relocated must not generate/drop its pending loot at the source. In Minecraft 26.2 the two-argument `setLootTable(ResourceKey<LootTable>, long)` boundary requires a non-null loot key, while the one-argument nullable loot-table-key setter is the current clear-key boundary. Therefore the exact compatibility adaptation is `setLootTable(null, 0)` -> `setLootTable(null)`; no non-null loot seed semantics are changed because the pending loot key is being removed.
- `scripts/apply_p1_relocationutil_loot_clear_26_2.py` changes exactly that one pinned call, requires exactly one old/new context, rejects the legacy two-argument null call afterward, and guards the adjacent source block-entity removal, both still-unmigrated `loadWithComponents` sites, already-frozen direct chunk writes, already-frozen neighbor dispatch, lighting checks, and the original conditional update flow against accidental absorption.
- Exact P1 run `35444914831` contains no compiler diagnostic for the former RelocationUtil loot-key nullability site and no new diagnostic at `setLootTable(null)`. Remaining RelocationUtil diagnostics are now only the two independent block-entity/component ValueInput/ValueOutput `loadWithComponents` boundaries.
- The prior RelocationUtil direct-chunk zero-flag and neighbor-update proofs remain frozen and unchanged: source AIR/destination rotated-state writes, ordering, explicit update flow, server/non-debug neighbor dispatch, update phases, and lighting checks are preserved.
- This proof does **not** authorize changes to container contents, non-null loot-table/seed behavior, block-entity/component ValueInput/ValueOutput, source removal or destination recreation ordering, direct chunk writes, neighbor updates, lighting policy, tickets, ship creation/allocation, transforms, physics, collision, entity dragging, rendering, networking authority, gameplay authority, or camera behavior.
- Scope reconciliation from prior ledger checkpoint `c4e12277a8d86229dbb668058ca1d68e7d60ebdc` to the proven implementation HEAD changed only the new fail-closed RelocationUtil loot-clear overlay and its canonical P1 workflow wiring; the pinned `upstream-vs2` gitlink was untouched.
...
The immediately prior implementation proof is `7a7e2f320605b27bb65e9f0bb6cb88d8aa41a4e7` (RelocationUtil neighbor-update boundary). Its proof and all earlier historical proof records/failed probes remain frozen evidence and must not be replayed merely because a later compiler error resembles them.
...
- active_proof_head: `52a05bdae75fd2a469287234c680bf191fd5b105; RelocationUtil pending loot-table clear complete and frozen`
- active_proof_run: `P0 35444914843 / job 105902069944 success; P1 35444914831 / job 105902069897 failure with the RelocationUtil loot-key nullability diagnostic cleared and no setLootTable(null) diagnostic; independent compiler clusters remain`
...
## Remaining compiler areas from exact run `35444914831`

These remain unresolved and independent from the frozen RelocationUtil loot-clear proof:
...
- `RelocationUtil`: only the two block-entity/component ValueInput/ValueOutput `loadWithComponents` boundaries remain. Its two direct `LevelChunk.setBlockState` flag arguments, two legacy `blockUpdated` neighbor-dispatch sites, and pending loot-table clear are **not** remaining compiler areas after the frozen proofs at `ce6ff8bb8f06eeb8e98544f6bf9ca1e503936fdb`, `7a7e2f320605b27bb65e9f0bb6cb88d8aa41a4e7`, and `52a05bdae75fd2a469287234c680bf191fd5b105`.

`AssemblyUtil.kt` has no remaining compiler diagnostic in exact run `35444914831`. The frozen ShipAssembler StructureProcessor boundary and all three frozen RelocationUtil boundaries are no longer remaining compiler areas.
...
3. Then inspect only exact run `35444914831` plus exact Minecraft 26.2 API/source for **one** remaining standalone-P1 compiler cluster. Do not patch multiple clusters in one step.
...
5. Do not mechanically extend the frozen RelocationUtil loot-clear proof into its remaining ValueInput/ValueOutput/component boundaries. Likewise do not mechanically extend prior RelocationUtil/ShipAssembler/AssemblyUtil changes into ticketing, renderer, authority-sensitive entity/networking, Sable, or Create compatibility. Choose exactly one root hypothesis only after semantic inspection, then use the smallest fail-closed traceable overlay and prove it separately.
