...
## Current reconciliation — RelocationUtil direct chunk-write flags proven and frozen

- Current proven implementation HEAD: `ce6ff8bb8f06eeb8e98544f6bf9ca1e503936fdb` (`P1: wire RelocationUtil chunk flag proof`).
- The implementation consists of fail-closed overlay commit `4496df0f425b0ab9a26af7dfcb2a37ceb3175a16` plus canonical P1 workflow wiring at `ce6ff8bb8f06eeb8e98544f6bf9ca1e503936fdb`; no upstream submodule commit or gameplay implementation was replaced.
- Exact-head P0 provenance run `35443067305`, job `105897176803`, completed `success`; the exact upstream gitlink remained `f39132148e717d325933b4ce6e9e9fb13d929390`.
- Exact-head P1 standalone compile run `35443067266`, job `105897176594`, completed `failure` only because independent Minecraft 26.2 source/API clusters remain. The RelocationUtil overlay step, port-delta validation, Gradle runtime setup, and compile-log artifact upload all completed successfully before/around the real `:common:compileKotlin` failure boundary.
- Diagnostic artifact: `p1-compile-log-ce6ff8bb8f06eeb8e98544f6bf9ca1e503936fdb`, artifact ID `10584129694`, size `5127` bytes, ZIP SHA-256 `9b77b280a4e22a9f93da1242142ee481019a899b80f024d1fca87d2bbaafeb5d`.
- Root cause proven for this isolated cluster: pinned upstream `RelocationUtil.relocateBlock()` rotates the block state, writes source AIR and destination state directly through `LevelChunk.setBlockState(BlockPos, BlockState, false)`, then conditionally invokes its own explicit `updateBlock(...)` flow. In the legacy direct-chunk API that third argument is the moved/isMoving Boolean. Minecraft 26.2 exposes the same direct `LevelChunk.setBlockState` boundary with an integer flag word; legacy `false` therefore maps to integer `0`, preserving no moved-by-piston bit and no additional direct-write flag bits while leaving the explicit `doUpdate` path unchanged.
- `scripts/apply_p1_relocationutil_chunk_setblock_flags_26_2.py` is fail-closed and targets exactly the one pinned relocation write/update context in `common/src/main/kotlin/org/valkyrienskies/mod/util/RelocationUtil.kt`. It changes only `fromChunk.setBlockState(from, AIR, false)` and `toChunk.setBlockState(to, state, false)` to the same calls with third argument `0`, requires exactly one full pinned context, rejects the legacy Boolean calls afterward, and guards the adjacent rotation, `doUpdate` call, block-entity/component calls, loot-table line, and the still-unmigrated legacy `blockUpdated` sites.
- Exact P1 run `35443067266` contains no compiler diagnostic for the former RelocationUtil Boolean-to-Int direct chunk-write sites and no diagnostic at either new zero-flag call. Remaining RelocationUtil diagnostics are independent: block-entity/component ValueInput/ValueOutput boundaries, loot-key nullability, and the two removed `blockUpdated` calls.
- This proof preserves the original rotated state, source/destination positions and states, write ordering, direct-chunk boundary, and conditional explicit update flow. It does **not** authorize changes to block-entity/component persistence, loot-table semantics, neighbor dispatch, lighting, relocation ordering, ship creation/allocation, transforms, physics, collision, entity dragging, rendering, networking authority, gameplay authority, or camera behavior.
- Scope reconciliation from prior ledger checkpoint `f856be46baa3112459ed593096eee50decdafae8` to the proven implementation HEAD changed only the new fail-closed RelocationUtil direct-chunk flag overlay and its canonical P1 workflow wiring; the pinned `upstream-vs2` gitlink was untouched.
...
The immediately prior implementation proof is `2da9ca25e3ce6fd2f601cc2f6b626b0e3998a677` (ShipAssembler StructureProcessor 26.2 adapter). Its proof and all earlier historical proof records/failed probes remain frozen evidence and must not be replayed merely because a later compiler error resembles them.
...
- active_proof_head: `ce6ff8bb8f06eeb8e98544f6bf9ca1e503936fdb; RelocationUtil direct chunk-write flags complete and frozen`
- active_proof_run: `P0 35443067305 / job 105897176803 success; P1 35443067266 / job 105897176594 failure with the two RelocationUtil Boolean-to-Int direct chunk-write diagnostics cleared; independent compiler clusters remain`
...
## Remaining compiler areas from exact run `35443067266`

These remain unresolved and independent from the frozen RelocationUtil direct chunk-write-flags proof:
...
- `RelocationUtil`: block-entity/component ValueInput/ValueOutput boundaries, loot-key nullability, and the two removed `blockUpdated` sites. Its two direct `LevelChunk.setBlockState` Boolean-to-Int flag arguments are **not** remaining compiler areas after the frozen proof at `ce6ff8bb8f06eeb8e98544f6bf9ca1e503936fdb`.

`AssemblyUtil.kt` has no remaining compiler diagnostic in exact run `35443067266`. The frozen ShipAssembler StructureProcessor boundary and the newly frozen RelocationUtil two direct chunk-write flag arguments are also no longer remaining compiler areas.
...
3. Then inspect only exact run `35443067266` plus exact Minecraft 26.2 API/source for **one** remaining standalone-P1 compiler cluster. Do not patch multiple clusters in one step.
...
5. Do not mechanically extend the frozen RelocationUtil zero-flag proof into its ValueInput/ValueOutput/component, loot-key, or `blockUpdated` boundaries. Likewise do not mechanically extend prior ShipAssembler/AssemblyUtil changes into ticketing, renderer, authority-sensitive entity/networking, Sable, or Create compatibility. Choose exactly one root hypothesis only after semantic inspection, then use the smallest fail-closed traceable overlay and prove it separately.
