...
## Current reconciliation — AssemblyUtil ScheduledTick non-null generic boundary proven and frozen

- Current proven implementation HEAD: `820ef37e8ca012fe484d9960242cbfc0eefbacc8` (`P1: wire AssemblyUtil ScheduledTick non-null proof`).
- Exact-head P0 provenance run `35436715908`, job `105880520574`, completed `success`; the exact upstream gitlink remained `f39132148e717d325933b4ce6e9e9fb13d929390`.
- Exact-head P1 standalone compile run `35436715912`, job `105880520583`, completed `failure` only because independent Minecraft 26.2 source/API clusters remain.
- Every overlay step through this proof, including `Apply traceable P1 AssemblyUtil ScheduledTick non-null overlay`, `Show and validate port delta`, and Gradle runtime setup completed successfully; compilation reached the real `:common:compileKotlin` boundary.
- Root cause proven for this isolated cluster: pinned upstream 1.21.1 `AssemblyUtil.copyBlock()` transfers a pending block tick through `ScheduledTick<Block?>(state.block, to, 0, 0)`. Minecraft 26.2 retains the same scheduled-tick constructor/scheduling flow, but Kotlin sees the `ScheduledTick` type parameter as non-null. The upstream value `state.block` is already a non-null `Block`, so the behavior-preserving adaptation is only the explicit generic bound `Block?` -> `Block`.
- `scripts/apply_p1_assemblyutil_scheduledtick_nonnull_26_2.py` is fail-closed and targets exactly the pinned-upstream scheduled-tick transfer context in:
  - `common/src/main/kotlin/org/valkyrienskies/mod/common/assembly/AssemblyUtil.kt`
- The overlay changes only `ScheduledTick<Block?>(state.block, to, 0, 0)` to `ScheduledTick<Block>(state.block, to, 0, 0)`. It requires exactly one pinned context, exactly one resulting non-null generic call, and rejects a remaining legacy nullable-generic call.
- This proof preserves the existing `hasScheduledTick(from, state.block)` guard, tick type `state.block`, destination `to`, trigger tick `0`, sub-tick order `0`, default constructor priority, and the existing `level.blockTicks.schedule(...)` flow. It does **not** change tick timing, priority, deduplication, transfer policy, block-entity/component persistence, `blockUpdated`, assembly ordering, relocation, ship allocation, transforms, physics, collision, networking, gameplay authority, or camera behavior.
- Exact run `35436715912` contains **no compiler diagnostic for the former `AssemblyUtil.kt` line-39 ScheduledTick nullable-generic mismatch**. Remaining `AssemblyUtil` diagnostics begin at independent line-44+ areas covering block-entity ValueInput/ValueOutput component persistence and `blockUpdated` API drift. This proof clears only the ScheduledTick explicit generic bound and does not make overall P1 green.
- Diagnostic artifact: `p1-compile-log-820ef37e8ca012fe484d9960242cbfc0eefbacc8`, artifact ID `10582521058`, size `5502` bytes, ZIP SHA-256 `cf6aeb758b898fb145f0d3202a94637f74adb4b1378f38cde5c19e97e6f00c9b`.
- Scope reconciliation from prior ledger checkpoint `d413b11f05d7ed92051b8207b6d3017b7660358a` to the proven implementation HEAD changed only the new fail-closed AssemblyUtil ScheduledTick non-null overlay and its canonical P1 workflow wiring; the pinned `upstream-vs2` gitlink was untouched.
...
The immediately prior implementation proof is `a803150d066fdc7e0a0bfdd5bd1661b85c627417` (AssemblyUtil direct LevelChunk setBlockState zero-flags boundary). Its proof and all earlier historical proof records/failed probes remain frozen evidence and must not be replayed merely because a later compiler error resembles them.
...
- active_proof_head: `820ef37e8ca012fe484d9960242cbfc0eefbacc8; AssemblyUtil ScheduledTick non-null generic boundary complete and frozen`
- active_proof_run: `P0 35436715908 / job 105880520574 success; P1 35436715912 / job 105880520583 failure with the AssemblyUtil line-39 ScheduledTick nullable-generic diagnostic cleared; remaining AssemblyUtil diagnostics begin at independent line-44+ clusters`
...
## Remaining compiler areas from exact run `35436715912`

These remain unresolved and independent from the frozen AssemblyUtil ScheduledTick non-null proof:
...
- `AssemblyUtil` line-44+ areas only: ValueInput/ValueOutput block-entity/component persistence and `blockUpdated` API drift. The line-39 `ScheduledTick<Block?>` explicit generic mismatch and the two direct `LevelChunk.setBlockState(..., false)` Boolean-to-Int flag sites are **not** remaining compiler areas.
...
`VSGameUtils.kt`, `VSGameEvents.kt`, the `AbstractArrow` / `AbstractHurtingProjectile` package sites, all three proven `ShipMountingEntity` adaptation boundaries (empty persistence hooks, `hurtServer(...)`, and server-side `kill(ServerLevel)`), the `ShipAssembler.getDistinctChunksFromBlockPosSet()` containing-chunk conversion, the two `AssemblyUtil` direct chunk-write flag arguments, and the `AssemblyUtil` ScheduledTick explicit generic bound are **not** remaining compiler areas after their frozen proofs.
